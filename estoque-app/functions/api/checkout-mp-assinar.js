...
export async function onRequestPost(context) {
  try {
    ...
    let corpo;
    try { corpo = await request.json(); } catch (e) { return json({ error: 'Corpo inválido.' }, 400); }
    ...
    const accessToken = env.MP_ACCESS_TOKEN_TEST || env.MP_ACCESS_TOKEN;
    ...
    if (ehTeste) {
      ...
      if (!mpRes.ok) {
        return json({ error: mpData.message || mpData.error || 'Erro ao processar pagamento.', mp_detalhe: mpData }, 502);
      }
    ...
    return json({ ok: true, status: mpStatus, planoAtivado: true });
